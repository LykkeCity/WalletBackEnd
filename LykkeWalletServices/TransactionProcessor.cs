...
using LykkeWalletServices.Transactions.TransactionScheduler;
using NBitcoin;
using System;
...
// This should also be used by ITransactionScheduler
...
namespace LykkeWalletServices
{
    public class TransactionProcessor : ITransactionProcessor
    {
        ...
        public static ITransactionScheduler TransactionScheduler
        {
            get;
            set;
        }

        public TransactionProcessor(IQueueWriter queueWriter, ILog log,
        ...
        public async Task Process(TransactionToDoBase @event)
        {
            ...
            if (knownTaskType)
            {
                await _log.WriteWarning("TransactionProcessor", "Execute", "", $"{@event.GetType()}");
            }
            else
            {
                await _log.WriteWarning("TransactionProcessor", "Execute", "", $"Unknown task type: {@event.GetType()}");
            }
        }
        ...
        private async Task ProcessTaskResult(TransactionToDoBase @event, TransactionResultModel resultModel)
        {
            ...
            {
                await _log.WriteFatalError("TransactionProcessor", string.Empty, string.Empty, e,
                    DateTime.UtcNow);
            }
        }
    }
